...
import { CheckoutData, PaymentCategory, ProductDetail, ProductVariant, PaymentMethod } from "../types.ts";
import { ChevronLeftIcon, ShareIcon, ChatBubbleLeftEllipsisIcon, StarIcon, ChevronDownIcon } from './icons/index.tsx';
import { mlProductPageBannerUrl } from './ImageAssets.ts';
import { formatDateOnlyShort } from "../src/lib/time.ts";


interface MLProductPageProps {
...
export default function MLProductPage({ product, paymentCategories, onProceedToCheckout, showToast, onBack }: MLProductPageProps) {
  ...
  const selectedVariant = React.useMemo(() => product.variants.find(d => d.id === selectedVariantId), [selectedVariantId, product.variants]);
  ...
  const fee = React.useMemo(() => {
    ...
    return Math.round((selectedPaymentMethod.fee.flat || 0) + subtotal * (selectedPaymentMethod.fee.percent || 0) / 100);
  ...
  function validate(): string | null {
    if (!uid.trim()) return "ID wajib diisi.";
    if (!server.trim()) return "Server wajib diisi.";
  ...
  return (
    <>
      <main className="max-w-screen-md mx-auto px-4 pb-[160px]">
        {/* Hero */}
        <section>
          <div className="w-full aspect-[1920/749] rounded-2xl bg-gradient-to-r from-[#7F1DFF] to-[#38BDF8] flex items-center justify-center text-white font-semibold translate-y-[5%]">
            <img src={mlProductPageBannerUrl} alt="Mobile Legends Banner" className="w-full h-full object-cover rounded-2xl"/>
          </div>
          ...
          <div className="mt-3 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
            <div className="flex gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-white">MOBILE LEGENDS – INDONESIA</h1>
        ...
        {tab==="trx" ? (
          <>
            ...
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">1 • Masukkan Data Akun</h3>
              ...
              <button className="mt-3 text-sm text-white">Cara cek ID?</button>
            </section>
            ...
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              ...
              <div className="mt-3 flex items-center gap-3">
                <button onClick={()=>setQty(q=>Math.max(1, q-1))}
                  className="h-10 w-10 text-2xl font-light rounded-xl border border-gray-600 bg-gray-700 text-white">−</button>
                                ...
                                <div className="p-3">
                                    <div className="space-y-2">
                                        {cat.methods.map(m => {
                                            ...
                                            const feeText = m.fee.flat > 0 && m.fee.percent > 0
            ...
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              ...
              <div className="mt-3 flex gap-2">
               ...
               <p className="text-xs text-gray-400 mt-2">Kode promo akan divalidasi di halaman checkout.</p>
            </section>
          ...
          <div className="space-y-4">
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">Deskripsi Mobile Legends – INDONESIA</h3>
              <p className="mt-2 text-gray-300">
                Top up diamond Mobile Legends cepat, aman, dan resmi di {APP_NAME}. Pilih nominal, isi ID & Server, pilih pembayaran, lalu pesan.
              </p>
              <ol className="mt-3 list-decimal list-inside space-y-1 text-gray-300">
                <li>Pilih nominal</li>
                <li>Masukkan data akun (ID & Server)</li>
