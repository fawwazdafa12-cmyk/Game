...
import { genshinProductPageBannerUrl, mlProductPageBannerUrl } from "./ImageAssets";
import { formatDateOnlyShort } from "../src/lib/time";

interface GenshinProductPageProps {
...
// UID: umumnya 9 digit; kita toleransi 8–10 digit
const RE_GI_UID = /^\d{8,10}$/;
// Hint region berdasar digit pertama (soft warning, tidak memblok)
const UID_REGION_HINT: Record<string, RegExp> = {
  "Asia": /^[89]\d{7,9}$/,
  "America": /^[56]\d{7,9}$/,
  "Europe": /^7\d{7,9}$/,
  "TW/HK/MO": /^5\d{7,9}$/,
...
export default function GenshinProductPage({
  product,
  paymentCategories,
  onProceedToCheckout,
  showToast,
  onBack,
}: GenshinProductPageProps) {
  ...
  const isUidValid = RE_GI_UID.test(uid.trim());
  ...
  function validate(): string | null {
    ...
    if (!isUidValid) return "UID harus angka 8–10 digit.";
  ...
  function handleOrder() {
    ...
    // Soft warning: UID vs server kemungkinan tidak cocok
    if (!regionHintOk) {
  ...
  const bannerSrc = genshinProductPageBannerUrl || mlProductPageBannerUrl;
      ...
      <main className="max-w-screen-md mx-auto px-4 pb-[160px]">
        ...
        <section>
          <div className="w-full aspect-[1920/749] rounded-2xl bg-gradient-to-r from-[#7F1DFF] to-[#38BDF8] overflow-hidden translate-y-[5%]">
            <img src={bannerSrc} alt="Genshin Impact Banner" className="w-full h-full object-cover rounded-2xl" />
          </div>
          ...
          <div className="mt-3 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
            <div className="flex gap-3">
              <div className="w-16 h-16 bg-gray-700 rounded-xl overflow-hidden">
                <img src={product.image} alt={product.name || "Genshin Impact"} className="w-full h-full object-cover" />
              </div>
              <div>
                <h2 className="text-lg font-bold text-white">GENSHIN IMPACT</h2>
        ...
        {tab === "trx" ? (
          <>
              ...
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onBlur={() => {
                      if (uid && !isUidValid) showToast("UID harus angka 8–10 digit.", "error");
                    }}
                  ...
                  <p className="text-xs text-gray-400 mt-2">
                    Cara cek UID: buka game → ketuk profil (kiri atas) → lihat UID (angka).
                  </p>
                </div>
                <div>
                  ...
                  {!regionHintOk && uid && server && (
                    <p className="text-xs text-red-400 mt-2">
                      UID tampaknya tidak sesuai dengan server {server}. Periksa kembali.
                    </p>
                  )}
                </div>
              </div>
            ...
            {/* Step 2: Nominal (Genesis Crystals / Blessing) */}
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">2 • Pilih Nominal (Genesis Crystals / Blessing)</h3>
            ...
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              ...
              {/* Opsional: jika variant adalah Blessing, batasi qty = 1 */}
              {/* {selectedVariant?.label.toLowerCase().includes('blessing') && <p className="text-xs text-yellow-400 mt-2">Blessing biasanya hanya dapat dibeli 1x. Pastikan status di akun kamu.</p>} */}
            </section>
          ...
          <div className="space-y-4">
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">Deskripsi Genshin Impact</h3>
              <p className="mt-2 text-gray-300">
                Top up Genesis Crystals dan Blessing of the Welkin Moon cepat, aman, dan resmi di {APP_NAME}.
              ...
              <ol className="mt-3 list-decimal list-inside space-y-1 text-gray-300">
                <li>Pilih nominal (Genesis Crystals / Blessing)</li>
                <li>Masukkan UID (angka 8–10 digit) dan pilih Server</li>
