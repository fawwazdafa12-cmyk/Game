...
import { aovProductPageBannerUrl, mlProductPageBannerUrl } from "./ImageAssets";
import { formatDateOnlyShort } from "../src/lib/time";

interface AOVProductPageProps {
...
// AOV: UID angka 6–15 digit (lebih fleksibel), Zone/Server angka 1–6 digit
const RE_AOV_UID = /^\d{6,15}$/;
const RE_AOV_ZONE = /^\d{1,6}$/;

function currency(n: number) {
  return new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(n);
}

export default function AOVProductPage({
  product,
  paymentCategories,
  onProceedToCheckout,
  showToast,
  ...
  const [zone, setZone] = useState(""); // Server/Zone ID
  ...
  const isUidValid = RE_AOV_UID.test(uid.trim());
  const isZoneValid = RE_AOV_ZONE.test(zone.trim());
  const isReady = Boolean(isUidValid && isZoneValid && selectedVariant && selectedPaymentMethod);

  function validate(): string | null {
    if (!uid.trim()) return "UID wajib diisi.";
    if (!isUidValid) return "UID harus angka 6–15 digit.";
    if (!zone.trim()) return "Server/Zone ID wajib diisi.";
    if (!isZoneValid) return "Server/Zone ID harus angka 1–6 digit.";
  ...
  function handleOrder() {
    ...
    if (selectedVariant && selectedPaymentMethod) {
      const checkoutData: CheckoutData = {
        ...
        formValues: {
          uid,          // AOV User ID
          server: zone, // simpan Zone ID di field server
          email,
          wa,
          promo,
        },
  ...
  const bannerSrc = aovProductPageBannerUrl || mlProductPageBannerUrl;
      ...
      <main className="max-w-screen-md mx-auto px-4 pb-[160px]">
        ...
        <section>
          <div className="w-full aspect-[1920/749] rounded-2xl bg-gradient-to-r from-[#7F1DFF] to-[#38BDF8] overflow-hidden translate-y-[5%]">
            <img src={bannerSrc} alt="Arena of Valor Banner" className="w-full h-full object-cover rounded-2xl" />
          </div>
          ...
          <div className="mt-3 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
            <div className="flex gap-3">
              <div className="w-16 h-16 bg-gray-700 rounded-xl overflow-hidden">
                <img src={product.image} alt={product.name || "Arena of Valor"} className="w-full h-full object-cover" />
              </div>
              <div>
                <h2 className="text-lg font-bold text-white">ARENA OF VALOR</h2>
                <p className="text-gray-400 text-sm">{product.brand || "Garena / TiMi Studio"}</p>
              </div>
            </div>
        ...
        {tab === "trx" ? (
          <>
            ...
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">1 • Masukkan UID & Server/Zone ID</h3>
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onBlur={() => { if (uid && !isUidValid) showToast("UID harus angka 6–15 digit.", "error"); }}
                  ...
                  <p className="text-xs text-gray-400 mt-2">
                    Cara cek UID: buka profil di game, salin angka UID kamu.
                  </p>
                </div>
                <div>
                  <label className="text-sm text-gray-400">Server/Zone ID</label>
                  <input
                    value={zone}
                    onChange={(e) => setZone(e.target.value.replace(/\D/g, "").slice(0, 6))}
                    onBlur={() => { if (zone && !isZoneValid) showToast("Server/Zone ID harus angka 1–6 digit.", "error"); }}
                    ...
                    className="mt-1 w-full h-11 rounded-xl border border-gray-700 bg-gray-900 text-white px-3 outline-none focus:ring-2 focus:ring-[#38BDF8]"
                  />
                  <p className="text-xs text-gray-400 mt-2">Server/Zone ID biasanya 3–4 digit.</p>
                </div>
              </div>
            </section>

            {/* Step 2: Nominal (Vouchers/Tokens/Pass) */}
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">2 • Pilih Nominal (Vouchers / Tokens / Pass)</h3>
            ...
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                <input
                  value={qty}
                  onChange={(e) =>
                    setQty(Math.max(1, parseInt((e.target.value || "1").replace(/\D/g, ""), 10) || 1))
                  }
                  type="text"
                  inputMode="numeric"
                  pattern="\d*"
          ...
          <div className="space-y-4">
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">Deskripsi Arena of Valor</h3>
              <p className="mt-2 text-gray-300">
                Top up Vouchers/Tokens/Pass AOV cepat, aman, dan resmi di {APP_NAME}. Pilih nominal, masukkan UID dan Server/Zone ID,
                pilih metode pembayaran, lalu pesan.
              </p>
              <ol className="mt-3 list-decimal list-inside space-y-1 text-gray-300">
                <li>Pilih nominal (Vouchers / Tokens / Pass)</li>
                <li>Masukkan UID (6–15 digit angka) & Server/Zone ID (1–6 digit)</li>
