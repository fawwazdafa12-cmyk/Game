...
import { CheckoutData, PaymentCategory, ProductDetail, ProductVariant, PaymentMethod } from "../types";
import { ChevronLeftIcon, ShareIcon, ChatBubbleLeftEllipsisIcon, StarIcon, ChevronDownIcon } from './icons';
import { ffProductPageBannerUrl, mlProductPageBannerUrl } from "./ImageAssets";
import { formatDateOnlyShort } from "../src/lib/time";

interface FFProductPageProps {
...
export default function FFProductPage({
  product,
  paymentCategories,
  onProceedToCheckout,
  showToast,
  ...
  function validate(): string | null {
    ...
    if (!id) return "Player ID wajib diisi.";
    if (!/^\d{5,20}$/.test(id)) return "Player ID harus angka 5–20 digit.";
  ...
  function handleOrder() {
    ...
    if (selectedVariant && selectedPaymentMethod) {
      const checkoutData: CheckoutData = {
        ...
        quantity: qty,
        formValues: {
          uid: playerId,   // gunakan field uid untuk Player ID
          server: "",      // FF tidak butuh server
          email: email,
          wa: wa,
          promo: promo,
        },
  ...
  const bannerSrc = ffProductPageBannerUrl || mlProductPageBannerUrl;
      ...
      <main className="max-w-screen-md mx-auto px-4 pb-[160px]">
          ...
          <div className="mt-3 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
            <div className="flex gap-3">
              <div className="w-16 h-16 bg-gray-700 rounded-xl overflow-hidden">
                <img
                  src={product.image}
                  alt={product.name || "Free Fire"}
                  className="w-full h-full object-cover"
                />
              </div>
              <div>
                <h2 className="text-lg font-bold text-white">FREE FIRE – INDONESIA</h2>
        ...
        {tab === "trx" ? (
          <>
              ...
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2">
                  ...
                  <p className="text-xs text-gray-400 mt-2">
                    Cara cek Player ID: buka game → profil (avatar) → lihat ID pemain.
                  </p>
                </div>
              </div>
            </section>
            ...
            {/* Step 2: Nominal */}
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">2 • Pilih Nominal (Diamonds/Bundle)</h3>
              ...
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="mt-1 flex gap-2">
                    <div className="h-11 px-3 rounded-xl border border-gray-700 flex items-center bg-gray-800 text-white">
                      +62
                    </div>
                    <input
                      type="tel"
                  ...
                  <p className="text-xs text-gray-400 mt-2">
                    Isi tanpa angka 0 di depan. Kami hubungi jika ada kendala.
                  </p>
                </div>
              </div>
            </section>
          ...
          <div className="space-y-4">
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">Deskripsi Free Fire – INDONESIA</h3>
              <p className="mt-2 text-gray-300">
                Top up Diamonds Free Fire cepat, aman, dan resmi di {APP_NAME}. Pilih nominal, masukkan
                Player ID, pilih metode pembayaran, lalu pesan.
              </p>
              <ol className="mt-3 list-decimal list-inside space-y-1 text-gray-300">
                <li>Pilih nominal Diamonds/Bundle</li>
                <li>Masukkan Player ID (tanpa spasi)</li>
                  ...
                  {[5, 4, 3, 2, 1].map((star) => (
                    <div key={star} className="flex items-center gap-2">
                      <span className="text-xs text-gray-400 flex items-center">
                        {star}
                        <StarIcon className="w-3 h-3 text-yellow-400 ml-0.5" />
                      </span>
                      <div className="w-full bg-gray-600 rounded-full h-1.5">
                        <div
                          className="bg-yellow-400 h-1.5 rounded-full"
                          style={{ width: `${(star / 5) * 80 + Math.random() * 20}%` }}
                        ></div>
                      </div>
                    </div>
                  ))}
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="flex">
                          {[...Array(5)].map((_, i) => (
                            <StarIcon
                              key={i}
                              className={`w-3.5 h-3.5 ${i < review.rating ? "text-yellow-400" : "text-gray-600"}`}
                            />
                          ))}
                        </div>
                      </div>
            ...
            <div className="text-sm text-gray-400 border border-dashed border-gray-700 rounded-xl p-3">
              {selectedVariant ? (
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span>
                    Item: <b className="text-white">{selectedVariant.label}</b> × {qty}
                  </span>
                  <span>
                    Total: <b className="text-white">{currency(grandTotal)}</b>
                  </span>
                </div>
              ) : (
