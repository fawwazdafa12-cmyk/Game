...
import { codmProductPageBannerUrl, mlProductPageBannerUrl } from "./ImageAssets";
import { formatDateOnlyShort } from "../src/lib/time";

interface CODMProductPageProps {
...
const TABBAR_H = 72;
const RE_CODM_OPENID = /^.{10,50}$/; // Open ID CODM: 10-50 karakter

function currency(n: number) {
  return new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(n);
}

export default function CODMProductPage({
  product,
  paymentCategories,
  onProceedToCheckout,
  showToast,
  onBack,
}: CODMProductPageProps) {
  ...
  const isUidValid = RE_CODM_OPENID.test(uid.trim());
  const isReady = Boolean(isUidValid && selectedVariant && selectedPaymentMethod);

  function validate(): string | null {
    if (!uid.trim()) return "Open ID wajib diisi.";
    if (!isUidValid) return "Format Open ID tidak valid.";
  ...
  const bannerSrc = codmProductPageBannerUrl || mlProductPageBannerUrl;
      ...
      <main className="max-w-screen-md mx-auto px-4 pb-[160px]">
        ...
        <section>
          <div className="w-full aspect-[1920/749] rounded-2xl bg-gradient-to-r from-[#7F1DFF] to-[#38BDF8] overflow-hidden translate-y-[5%]">
            <img src={bannerSrc} alt="COD Mobile Banner" className="w-full h-full object-cover rounded-2xl" />
          </div>
          ...
          <div className="mt-3 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
            <div className="flex gap-3">
              <div className="w-16 h-16 bg-gray-700 rounded-xl overflow-hidden">
                <img src={product.image} alt={product.name || "Call of Duty Mobile"} className="w-full h-full object-cover" />
              </div>
              <div>
                <h2 className="text-lg font-bold text-white">CALL OF DUTY: MOBILE</h2>
                <p className="text-gray-400 text-sm">{product.brand || "Activision / Garena"}</p>
              </div>
            </div>
        ...
        {tab === "trx" ? (
          <>
            ...
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">1 • Masukkan Open ID</h3>
              <div className="mt-3">
                <label className="text-sm text-gray-400">Open ID</label>
                <input
                  value={uid}
                  onChange={(e) => setUid(e.target.value)}
                  onBlur={() => { if (uid && !isUidValid) showToast("Format Open ID tidak valid.", "error"); }}
                ...
                <p className="text-xs text-gray-400 mt-2">
                  Cara cek Open ID: buka game → Settings → Legal and Privacy → lihat di bawah.
                </p>
              </div>
            </section>
          ...
          <div className="space-y-4">
            <section className="mt-4 p-4 bg-gray-800 border border-gray-700 rounded-2xl">
              <h3 className="font-bold text-white">Deskripsi Call of Duty: Mobile</h3>
              <p className="mt-2 text-gray-300">
                Top up CP CODM cepat, aman, dan resmi di {APP_NAME}. Pilih nominal, masukkan Open ID, pilih
                metode pembayaran, lalu pesan.
              </p>
              <ol className="mt-3 list-decimal list-inside space-y-1 text-gray-300">
                <li>Pilih nominal (CP / Bundle / Pass)</li>
                <li>Masukkan Open ID (10-50 karakter)</li>
